...
if choice == "Adding":
    ...
    if submit_btn:
        ...
        if name == None and image == None:
            st.error("Driver ID does not exist")
        else:
            st.success(f"Name of Driver with ID {id} is: {name}")
            st.warning("Please check the image below to make sure you are deleting the right Driver")
            st.image(image)
    ...
    if submit_btn:
        ...
        if old_name == None and old_image == None:
            ...
            with st.form(key='my_form'):
                st.title("Adjusting Driver info")
